	...
	static MobVacuum(entity) {
		...
		if (this.isMonster(entity) && this.isIndistance(entity)) {
			...
			timer = setInterval(() => {
				...
				its++;
				let distToPlayer = ModUtils_1.ModUtils.Getdistance2Player(
					this.GetPosition(entity.Entity),
				);
				if (distToPlayer < 500) {
					clearInterval(timer);
					return;
				...
				let playerDistToSpawn = ModUtils_1.ModUtils.Getdistance(
					this.origPositions[entityId],
					playerpos,
				);
				if (
					playerDistToSpawn >
					ModManager_1.ModManager.Settings.VacuumRadius * 100
				) {
					clearInterval(timer);
					return;
				}
