...
class PlotConfig {
	...
	SetMode(e, t = !1) {
		switch (
			((this.SkipHiddenBlackScreenAtEnd = e.DisableAutoFadeOut),
			...
			case "LevelA":
				(this.CameraMode = 0),
					(this.CanInteractive = !1),
					(this.CanSkip = !e.NoSkip || this.CanSkipDebug),
					(this.DisableInput = !0),
					(this.CanPause = !1),
			...
			case "LevelB":
				...
				break;
			case "LevelC":
				void 0 === e.UseFlowCamera
					? (this.CameraMode = 2)
					: (this.CameraMode = e.UseFlowCamera ? 2 : 1),
					(this.CanInteractive = !0),
					(this.CanSkip = !e.NoSkip || this.CanSkipDebug),
					(this.DisableInput = !0),
					(this.CanPause = !t),
					...
					(this.ShouldSwitchMainRole = e.IsSwitchMainRole),
					(this.PauseTime = !t),
					(this.SkipTalkWhenFighting = !1),
					PlotController_1.PlotController.TogglePlotProtect(!0);
				break;
			case "LevelD":
				(this.CameraMode = 1),
					...
					(this.PauseTime = !1),
					(this.SkipTalkWhenFighting = e.Interruptible),
...
class PlotModel extends ModelBase_1.ModelBase {
	...
	PendingPlot(e) {
		for (let o = this.PlotPendingList.length - 1; 0 <= o; o--) {
			var t = this.PlotPendingList[o];
			if ("LevelD" === t.PlotLevel)
				Log_1.Log.CheckInfo() &&
					Log_1.Log.Info(
						"Plot",
						27,
			...
			t.IsServerNotify ? (t.IsBackground = !0) : this.PlotPendingList.pop();
		}
	...
	SetPendingPlotState(e, t, o, i) {
		for (const l of this.PlotPendingList)
			if (l.FlowIncId === e)
				return (
					(l.IsBackground = o), (l.IsBreakdown = t), (l.IsServerEnd = i), !0
				);
		return !1;
	}
	CenterTextTransition(e, t) {
		this.FYi === e
			? t && t()
			: (this.FYi = e)
				? LevelLoadingController_1.LevelLoadingController.OpenLoading(
						...
						() => {
							PlotController_1.PlotController.ClearUi(),
								UiManager_1.UiManager.OpenView("PlotTransitionView", void 0, t);
						},
						0.5,
					)
	...
	ShowTalkCenterText(e, t) {
		e
			? ((this.CenterText.Text = PublicUtil_1.PublicUtil.GetFlowConfigLocalText(
					e.TidTalk,
				)),
				(this.CenterText.AudioId = e.PlayVoice ? e.TidTalk : ""),
				(this.CenterText.Config = e.CenterTextConfig),
				(this.CenterText.AutoClose = !0),
				(this.CenterText.TalkAkEvent = e.TalkAkEvent),
				(this.CenterText.UniversalTone = e.UniversalTone),
	...
	SetInPlotGameBudget(e) {
		this.HasSetGameBudget !== e &&
			((this.HasSetGameBudget = e),
	...
	SetTemplatePlayerTransform(e) {
		this.PlotTemplate.IsInTemplate &&
			this.PlotTemplate.SetTemplatePlayerTransform(e);
	}
	StartPlotTemplate(e, t, o) {
		this.PlotTemplate.StartTemplateNew(e, t, o);
	}
	SetPlotTemplate(e, t) {
		this.PlotTemplate.SetTemplateNew(e).finally(t);
	}
	EndPlotTemplate(e, t) {
		this.PlotTemplate.EndTemplateNew(e).finally(t);
	}
	SetActorName(e) {
		this.PlotTemplate.IsInTemplate && this.PlotTemplate.SetActorName(e);
	}
	PlayCameraAnim(e) {
		this.PlotTemplate.IsInTemplate &&
			3 === this.PlotConfig.CameraMode &&
			this.PlotTemplate.PlayCameraAnimCompatible(e);
	}
	HYi() {
		var e;
		"LevelD" !== this.PlotConfig.PlotLevel &&
			...
			e.StopAllSkills("PlotModel.StopMainCharacterSkill");
	}
	KYi() {
		InputDistributeController_1.InputDistributeController.RefreshInputTag();
	}
	SwitchCameraMode(e) {
		(this.PlotConfig.CameraMode = e), this.WYi();
	}
	WYi(e = !1) {
		if (GlobalData_1.GlobalData.World)
			switch (this.PlotConfig.CameraMode) {
				case 0:
					...
					t?.GetIsInCinematic() && t.StopSequence(),
						t?.ResetCameraRatioSetting();
					break;
				case 1:
	...
	MarkGrayOption(e, t) {
		this.GrayOptionMap.has(e) || this.GrayOptionMap.set(e, new Set()),
			(e = this.GrayOptionMap.get(e)).has(t) || e.add(t);
	}
	IsOptionGray(e, t) {
		return !!this.GrayOptionMap.has(e) && !!this.GrayOptionMap.get(e).has(t);
	}
	CheckOptionCondition(e, t) {
		if (!e.PreCondition) return !0;
		let o = !1;
		return "PreOption" === e.PreCondition.Type ? this.YYi(e, t) : o;
	}
	YYi(e, t) {
		let o = !0;
		for (const l of e.PreCondition.PreOptions) {
			var i = this.GrayOptionMap.get(t.Id);
			o = o && !!i && i.has(l);
		}
		return o;
	}
	GetOptionIndex(e, t) {
		return (
			this.CurShowTalk?.TalkItems.find((e) => e.Id === t)?.Options?.indexOf(
				e,
			) ?? -1
		);
	}
	...
	JYi() {
		...
		if (
			e &&
			((e = EntitySystem_1.EntitySystem.Get(e)?.GetComponent(29)), e?.Valid)
		)
			return e;
	}
	ClearContext() {
	...
	InitPlotTemplate() {
		...
		if (
			...
			for (const e of JSON.parse(t).List) this.NYi.set(e.Id, e);
	}
	InitMontageConfig() {
		this.jZ = new Map();
		let e = (0, PublicUtil_1.getConfigPath)(
		...
		if (
			...
			for (const e of JSON.parse(t).Montages) this.jZ.set(e.Id, e);
	}
	GetMontageConfig(e) {
		if (PublicUtil_1.PublicUtil.UseDbConfig()) {
			if ((this.jZ || (this.jZ = new Map()), !this.jZ.get(e))) {
				...
				(t = {
					Id: t.Id,
					ActionMontage: t.ActionMontage,
					ExpressionMontage: t.ExpressionMontage,
					MouthSequence: t.MouthSequence,
				}),
					this.jZ.set(e, t);
			}
	...
	QZ() {
		...
		if (
			...
			for (const e of JSON.parse(t).Montages) this.WZ.set(e.Id, e);
	}
	GetAbpMontageConfig(e) {
		if (PublicUtil_1.PublicUtil.UseDbConfig()) {
	...
	zYi() {
		...
		if (
			...
			for (const e of JSON.parse(t).Montages) this.OYi.set(e.Id, e);
	}
	GetOverlayAbpMontageConfig(e) {
		...
		return this.OYi.get(e);
	}
	GetPlotTemplateConfig(e) {
		if (PublicUtil_1.PublicUtil.UseDbConfig()) {
			if ((this.NYi || (this.NYi = new Map()), !this.NYi.get(e))) {
				...
				(t = {
					...
					ActorDataArray: JSON.parse(t.ActorDataArray),
					CameraData: JSON.parse(t.CameraData),
				}),
					this.NYi.set(e, t);
			}
		} else this.NYi || this.InitPlotTemplate();
		return this.NYi.get(e);
	}
}
