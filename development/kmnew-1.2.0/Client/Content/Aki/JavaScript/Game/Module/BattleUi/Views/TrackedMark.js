...
class TrackedMark extends UiPanelBase_1.UiPanelBase {
	constructor(t) {
		super(),
			...
			(this.egt = (t, e, i, a, r) => {
				t === this.ECt &&
					a === this.MCt &&
					((this.IsInTrackRange = r), this.BCt?.SetUIActive(!r));
			}),
			GlobalData_1.GlobalData.World &&
				...
				(this.y$e = Math.min(1176, ((t?.GetWidth() ?? 0) - 1008) / 2)),
				(this.I$e = Math.min(712.5, ((t?.GetHeight() ?? 0) - 495) / 2)),
	...
	OnStart() {
		(this.xCt = this.GetItem(4)),
			...
			5 === this.ECt && this.RootItem?.SetHierarchyIndex(999),
			this.CreateMark(),
			this.OnUiShow();
	}
	OnBeforeDestroy() {
	...
	OnUiShow() {
		...
		1 === this.WCt &&
			...
			(this.SCt = TimerSystem_1.TimerSystem.Delay(this.YCt, 500)),
			EventSystem_1.EventSystem.Has(
				EventDefine_1.EEventName.OnLogicTreeNodeStatusChange,
				this.JCt,
			) ||
	...
	UpdateTrackDistance() {
		...
		t &&
			...
			ModManager_1.ModManager.Settings.QuestTp &&
				((ModManager_1.ModManager.Settings.QuestX = this.TempTrackPosition.X),
				(ModManager_1.ModManager.Settings.QuestY = this.TempTrackPosition.Y),
				(ModManager_1.ModManager.Settings.QuestZ = this.TempTrackPosition.Z));
	}
	Update(t) {
		var e;
		GlobalData_1.GlobalData.World
			? UiLayer_1.UiLayer.UiRootItem
				? this.RootItem &&
					...
					this.tgt()
						? (e = this.LCt) < this.MarkHideDis && !this.PCt
	...
	UpdatePositionAndRotation(t) {
		...
		r ||
			...
			UE.GameplayStatics.ProjectWorldToScreen(i, e, this.ScreenPositionRef)),
			(a = (0, puerts_1.$unref)(this.ScreenPositionRef));
		this.ScreenPosition.Set(a.X, a.Y),
			(this.LastScreenPosition.Equals(this.ScreenPosition, 1) &&
				...
				Info_1.Info.IsInTouch() || i.UpdateViewPortSize(),
				this.ScreenPosition.MultiplyEqual(i.ScreenPositionScale)
				...
				(e = this.ScreenPosition.AdditionEqual(center)),
				...
				this.InRange || 1 !== this.WCt
					? this.bCt.SetNiagaraVarFloat("Rotation", 0.25)
					: ((a =
							Math.atan2(this.ScreenPosition.Y, this.ScreenPosition.X) /
							(2 * Math.PI)),
						this.bCt.SetNiagaraVarFloat("Rotation", a)));
	}
	MoveTowards(t, e, i) {
		var a = e.X - t.X,
	...
	tgt() {
		return (
			!this.pCt &&
			!(this.ACt && this.CurShowTime > this.kCt) &&
				!(
	...
	ClampToEllipse(t, e) {
		...
		return (
			!!(e && (i * i) / (r * r) + (a * a) / (o * o) <= 1) ||
			((e = (r * o) / Math.sqrt(o * o * i * i + r * r * a * a)),
			t.MultiplyEqual(e),
			!1)
		);
	}
	ogt(t) {
		1 === this.WCt &&
			...
			(this.NiagaraNeedActivateNextTick &&
				((this.NiagaraNeedActivateNextTick = !1),
				this.bCt.SetNiagaraVarFloat("LifeTime", this.GCt),
				this.LCt <= this.KCt
					? (this.bCt.SetNiagaraVarFloat("Scale", 1),
						this.bCt.SetNiagaraVarLinearColor(
							"Color",
							new UE.LinearColor(UE.Color.FromHex("86FF83")),
						))
					: this.LCt > this.KCt && this.LCt <= this.QCt
						? (this.bCt.SetNiagaraVarFloat("Scale", 2 / 3),
							this.bCt.SetNiagaraVarLinearColor(
								"Color",
						...
						: this.LCt > this.QCt &&
							this.LCt <= this.XCt &&
							(this.bCt.SetNiagaraVarFloat("Scale", 1 / 3),
							this.bCt.SetNiagaraVarLinearColor(
								"Color",
